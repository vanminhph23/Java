...
public interface PatientHistoryRepository extends BaseRepository<PatientHistory, Long> {
    ...
    @Query("select e from PatientHistory e where e.idNo = ?1")
    List<PatientHistory> findByIdNo(String idNo, Pageable pageable);

    @Transactional(readOnly = true)
    @Query("select e from PatientHistory e where e.patientValue = ?1")
    List<PatientHistory> findByPatientValue(String patientValue, Pageable pageable);

    @Transactional(readOnly = true)
    @Query("select e from PatientHistory e where e.patientValue = ?1 and e.inpatient = true and e.patientState in ?2 and e.id <> ?3")
